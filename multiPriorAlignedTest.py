...
DEFAULT_EXP_ROOT = Path("/local/scratch/tpiltne/models/multiPriorAligned")
...
def collate_multi_prior_aligned(batch):
    """
    Expected examples:
      (cur, ali, years, hpv, y, m) [+ optional group_id]
    ...
    has_group = len(batch[0]) >= 7

    if has_group:
        cur_list, ali_list, years_list, hpv_list, y_list, m_list, gid_list = zip(*batch)
    else:
        cur_list, ali_list, years_list, hpv_list, y_list, m_list = zip(*batch)
        gid_list = None

    B = len(batch)
    Kmax = max(int(a.shape[0]) for a in ali_list) if B > 0 else 0
    ...
    if Kmax == 0:
        ...
        if has_group:
            ...
            return cur, ali, years, pad, hpv, y, m, gid
        return cur, ali, years, pad, hpv, y, m

    ali_pad = torch.zeros((B, Kmax, 4, 1, cur.shape[-2], cur.shape[-1]), dtype=cur.dtype)
    years_pad = torch.zeros((B, Kmax), dtype=torch.float32)
    pad_mask = torch.ones((B, Kmax), dtype=torch.bool)
    hpv_pad = torch.zeros((B, Kmax, 4), dtype=torch.float32)

    for i in range(B):
        Ki = int(ali_list[i].shape[0])
        if Ki == 0:
            continue
        ali_pad[i, :Ki] = ali_list[i]
        years_pad[i, :Ki] = years_list[i].to(torch.float32)
        hpv_pad[i, :Ki] = hpv_list[i].to(torch.float32)
        pad_mask[i, :Ki] = False

    if has_group:
        gid = torch.as_tensor(gid_list)
        return cur, ali_pad, years_pad, pad_mask, hpv_pad, y, m, gid
    return cur, ali_pad, years_pad, pad_mask, hpv_pad, y, m
...
def build_model_from_cfg(cfg: Dict[str, Any]) -> torch.nn.Module:
    ...
    diff_mode = str(cfg.get("diff_mode", "abs"))
    ...
    return MultiPriorRiskAlignedWrapper(
...
def make_loader(dataset: torch.utils.data.Dataset) -> DataLoader:
    return DataLoader(
        ...
        collate_fn=collate_multi_prior_aligned,
    )
...
def collect_logits_labels_masks(
    ...
    for batch in loader:
        if not isinstance(batch, (list, tuple)) or len(batch) < 7:
            raise ValueError("Unexpected batch format from collate_multi_prior_aligned.")

        cur, ali, years, pad, hpv, y, m = batch[:7]
        group_ids = batch[7] if len(batch) >= 8 else None
        ...
        try:
            out = model(cur, ali, pri_years=years, pri_pad_mask=pad, has_prior_views=hpv)
        except TypeError:
            out = model(cur, ali, years, pad, hpv)
...
def resolve_exp_dir(exp_dir: Optional[Path]) -> Path:
    ...
    patterns = ["multiPriorAligned_grid_then_final_*", "multiPrior_grid_then_final_*"]
    runs = []
    for pat in patterns:
        runs.extend(sorted(exp_dir.glob(pat), key=lambda p: p.name))
    if not runs:
...
def main():
    ...
    eval_dir = exp_dir / f"eval_multiprior_aligned{run_tag}"
    ...
    test_ds = TemporalMultiPriorAllAlignedDataset(split="test")

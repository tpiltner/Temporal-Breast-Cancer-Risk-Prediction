...
DEFAULT_EXP_ROOT = Path("/local/scratch/tpiltne/models/temporalModelNotAligned")
...
def metric_summary(
    ...
    if auc_point is not None:
        auc_samples = bootstrap_metric_samples(
            y, s,
            lambda yy, ss: safe_auc_auprc(yy.astype(int), ss)[0],
            groups=groups,
        )
    ...
    if ap_point is not None:
        ap_samples = bootstrap_metric_samples(
            y, s,
            lambda yy, ss: safe_auc_auprc(yy.astype(int), ss)[1],
            groups=groups,
        )
...
def plot_uq_boxplots(
    ...
    for h in RISK_COLS:
        for subset in order:
            ...
            if point is not None:
                plt.scatter([positions[point_idx]], [point], marker="o", s=55, zorder=3)
            point_idx += 1
    ...
    legend_elements = [
        Patch(facecolor=COLOR_OVERALL, edgecolor="black", alpha=0.35, label="overall"),
        Patch(facecolor=COLOR_ACCEPTED, edgecolor="black", alpha=0.35, label=ACCEPTED_CLIN_NAME),
        Patch(facecolor=COLOR_REJECTED, edgecolor="black", alpha=0.35, label=REJECTED_CLIN_NAME),
    ]
    plt.legend(handles=legend_elements, loc="lower right", fontsize=LEGEND_FONTSIZE, framealpha=0.95)
...
def resolve_exp_dir(exp_dir: Optional[Path]) -> Path:
    ...
    runs = sorted(exp_dir.glob("temporal_grid_then_final_*"), key=lambda p: p.name)
    if not runs:
        raise FileNotFoundError(f"No temporal_grid_then_final_* runs found under: {exp_dir}")
    return runs[-1]
...
def main():
    ...
    eval_dir = exp_dir / f"eval_temporal_noalign{run_tag}"
    ...
    test_ds = Temporal1Dataset(split="test")

...
from datasetModel import Temporal1Dataset, RISK_COLS
from modelArchitecture import RiskModel_no_alignment


EXP_ROOT = Path("/local/scratch/tpiltne/models/temporalModelNotAligned")
...
def evaluate(
    ...
    for imgs, delta_feat, has_prior_views, y, m in loader:
        ...
        if USE_AUX_LOSSES:
            if "pred_cur" in out:
                loss = loss + float(AUX_W_CUR) * masked_bce_with_logits(
                    out["pred_cur"], y, m, pos_weight=pos_weight
                )
            if "pred_pri" in out:
                loss = loss + float(AUX_W_PRI) * masked_bce_with_logits(
                    out["pred_pri"], y, m, pos_weight=pos_weight
                )
...
def build_optimizer(
    ...
    if (not unfreeze_encoder) or (len(enc_params) == 0):
        ...
        params_for_clip = head_params
        return opt, params_for_clip
    ...
    params_for_clip = head_params + enc_params
    return opt, params_for_clip
    ...
    for nh in GRID_NUM_HEADS:
        for do in GRID_DROPOUT:
            for lr in GRID_LR:
                for wd in GRID_WD:
                    grid.append(
                        {"num_heads": int(nh), "dropout": float(do), "lr": float(lr), "wd": float(wd)}
                    )
    return grid
...
def train_one_config_grid(cfg: Dict[str, Any], run_dir: Path, device: torch.device) -> Dict[str, Any]:
    ...
    train_ds = Temporal1Dataset(split="train")
    val_ds = Temporal1Dataset(split="val")

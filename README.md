...
## Abstract 
Breast cancer is one of the most common diseases and leading causes of mortality in women. Radiologists increasingly use AI systems as a second opinion to enhance screening accuracy from mammography exams. However, most deep learning models rely on a single scan and overlook longitudinal comparisons routinely used in clinical practice. A key challenge in using prior mammograms is that patient positioning and compression can make temporal comparisons unreliable without anatomical alignment. Furthermore, existing models rarely communicate predictive uncertainty, which may encourage over-reliance on outputs even in ambiguous cases. Limited research has explored approaches that include both patients' complete longitudinal mammogram scans and precisely spatially aligning these scans, as well as comparing and evaluating the contributions of each to predictive performance. We conduct a comprehensive comparison of mammography screening models with multiple priors, spatial alignment, and uncertainty quantification. Using the EMBED Open dataset, we trained models that predict the risk of breast cancer developing within one to five years by combining a patient’s current mammogram with all available prior scans, spatially aligning these exams over time, and quantifying model uncertainty using entropy-based measures. The models refer uncertain cases to clinician for additional review. We compare the performance of five deep learning models to answer: (i) does temporal modeling improve future cancer risk predictions; (ii) does temporal alignment improve future cancer risk predictions; and (iii) do uncertainty quantified decision referrals help with clinician decision making? Our results demonstrate that spatially aligning a prior exam yields a statistically significant improvement ($p < 0.05$) in precision-recall performance (AUPRC) across all prediction horizons compared to using an unaligned prior exam. Temporal models with aligned priors also showed favorable point estimates relative to the current-exam baseline, although these differences were not statistically significant on this dataset. Incorporating multiple priors did not consistently improve performance over a single aligned prior. The results suggest that spatial alignment helps capture more meaningful longitudinal changes and, for longitudinal mammography risk prediction, one aligned recent prior may be more informative than unaligned or multiple prior exams. Uncertainty-based referral has potential to support clinician review but only weakly separates more reliable from less reliable cases in this setting. 

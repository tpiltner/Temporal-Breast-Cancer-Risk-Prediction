...
from datasetModel import TemporalMultiPriorDataset, RISK_COLS
from modelArchitecture import MultiPriorRisk


EXP_ROOT = Path("/local/scratch/tpiltne/models/multiPrior")
...
# Early stopping (final only)
EARLY_STOP_PATIENCE = 3  

# Selection horizon indices
AUC3TO5_IDXS = [2, 3, 4]  # horizons for years 3..5
...
# Collate: pad variable K priors
def collate_multi_prior(batch):
    """
    Each item:
      cur: [4,1,H,W]
      pri: [K,4,1,H,W]
    ...
    Returns:
      ...
      pri: [B,Kmax,4,1,H,W]
      t:   [B,Kmax]
      pad: [B,Kmax]  True=pad/ignore
      y:   [B,5]
      m:   [B,5]
    """
    cur_list, pri_list, t_list, y_list, m_list = zip(*batch)
    B = len(batch)

    Kmax = max(int(p.shape[0]) for p in pri_list) if B > 0 else 0
    device = cur_list[0].device if torch.is_tensor(cur_list[0]) else None 

    # shapes
    cur = torch.stack(cur_list, dim=0)  # [B,4,1,H,W]
    y = torch.stack(y_list, dim=0)      # [B,5]
    m = torch.stack(m_list, dim=0)      # [B,5]

    if Kmax == 0:
        pri = torch.zeros((B, 0, 4, 1, cur.shape[-2], cur.shape[-1]), dtype=cur.dtype)
        t = torch.zeros((B, 0), dtype=torch.float32)
        pad = torch.zeros((B, 0), dtype=torch.bool)
        return cur, pri, t, pad, y, m
    ...
    for i in range(B):
        pri_i = pri_list[i]  # [Ki,4,1,H,W]
        t_i = t_list[i]      # [Ki]
        Ki = int(pri_i.shape[0])

        # pad pri
        if Ki < Kmax:
            pad_amt = Kmax - Ki
            pri_i = torch.cat(
                [pri_i, torch.zeros((pad_amt, *pri_i.shape[1:]), dtype=pri_i.dtype)],
                dim=0,
            )
            t_i = torch.cat([t_i, torch.zeros((pad_amt,), dtype=torch.float32)], dim=0)
            pm = torch.cat([torch.zeros((Ki,), dtype=torch.bool), torch.ones((pad_amt,), dtype=torch.bool)], dim=0)
        else:
            pm = torch.zeros((Kmax,), dtype=torch.bool)

        pri_pad.append(pri_i)
        t_pad.append(t_i)
        pad_mask.append(pm)

    pri = torch.stack(pri_pad, dim=0)          # [B,Kmax,4,1,H,W]
    t = torch.stack(t_pad, dim=0).to(torch.float32)  # [B,Kmax]
    pad = torch.stack(pad_mask, dim=0)         # [B,Kmax] True=pad
    return cur, pri, t, pad, y, m
...
def compute_pos_weight_from_train(
    ...
    if hasattr(train_ds, "exam_groups") and isinstance(train_ds.exam_groups, list) and len(train_ds.exam_groups) > 0:
        for exam_id, df_exam, y_event, mask, hw in train_ds.exam_groups:
            ...
            m = torch.as_tensor(np.asarray(mask), dtype=torch.float64)
            tot_pos += (y * m)
            tot_neg += ((1.0 - y) * m)
    else:
        tmp_loader = DataLoader(train_ds, batch_size=32, shuffle=False, num_workers=0, collate_fn=collate_multi_prior)
        for cur, pri, t, pad, y, m in tmp_loader:
...
def masked_bce_with_logits(
    ...
    mask: torch.Tensor,
    pos_weight: Optional[torch.Tensor] = None,  # [T]
) -> torch.Tensor:
...
def enforce_encoder_bn_eval(model: nn.Module):
    ...
    if enc is None:
        # fallback: search a module named encoder
...
def evaluate(
    ...
    for cur, pri, t, pad, y, m in loader:
        cur = cur.to(device, dtype=torch.float32)
        pri = pri.to(device, dtype=torch.float32)
        t = t.to(device, dtype=torch.float32)
        pad = pad.to(device)  # bool
        y = y.to(device, dtype=torch.float32)
        m = m.to(device, dtype=torch.float32)

        out = model(cur, pri, pri_years=t, pri_pad_mask=pad)["risk_prediction"]
        logits = out["pred_fused"]  # [B,5]
...
def train_one_config_grid(cfg: Dict[str, Any], run_dir: Path, device: torch.device) -> Dict[str, Any]:
    ...
    run_cfg.update(
        {
            ...
            "model": "MultiPriorRisk",
        }
    )
    (run_dir / "run_config.json").write_text(json.dumps(run_cfg, indent=2))

    train_ds = TemporalMultiPriorDataset(split="train")
    val_ds = TemporalMultiPriorDataset(split="val")
    ...
    train_loader = DataLoader(
        ...
        drop_last=False,
        collate_fn=collate_multi_prior,
        **_dl_kwargs(),
    )
    ...
    for epoch in range(GRID_EPOCHS):
        ...
        for step, (cur, pri, t, pad, y, m) in enumerate(train_loader):
            cur = cur.to(device, dtype=torch.float32)     # [B,4,1,H,W]
            pri = pri.to(device, dtype=torch.float32)     # [B,K,4,1,H,W]
            t = t.to(device, dtype=torch.float32)         # [B,K]
            pad = pad.to(device)                          # [B,K] bool
            y = y.to(device, dtype=torch.float32)         # [B,5]
            m = m.to(device, dtype=torch.float32)         # [B,5]
...
def train_final(best_cfg: Dict[str, Any], final_dir: Path, device: torch.device) -> Dict[str, Any]:
    ...
    for epoch in range(FINAL_EPOCHS):
        ...
        for step, (cur, pri, t, pad, y, m) in enumerate(train_loader):
            ...
            with torch.cuda.amp.autocast(enabled=USE_AMP):
                #out = model(cur, pri, t, pri_pad_mask=pad)["risk_prediction"]
                out = model(cur, pri, pri_years=t, pri_pad_mask=pad)["risk_prediction"]
                logits = out["pred_fused"]
                loss = masked_bce_with_logits(logits, y, m, pos_weight=pos_weight)
                loss = loss / float(GRAD_ACCUM_STEPS)
...
def main():
    ...
    if args.exp_dir is not None:
        ...
        stamp = (
            exp_dir.name.split("multiPrior_grid_then_final_")[-1]
            if "multiPrior_grid_then_final_" in exp_dir.name
            else datetime.now().strftime("%Y%m%d_%H%M%S")
        )
    else:
        stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        exp_dir = EXP_ROOT / f"multiPrior_grid_then_final_{stamp}"
        exp_dir.mkdir(parents=True, exist_ok=True)
    ...
    if not exp_cfg_path.exists():
        exp_cfg = {
            ...
            "model": "MultiPriorRisk",
            "dataset": "TemporalMultiPriorDataset",
        }
    ...
    print(
        f"[INFO] GRID freeze_encoder={FREEZE_ENCODER_GRID} | FINAL unfreeze_encoder={UNFREEZE_ENCODER_FINAL} "
        f"(encoder_lr_mult={ENCODER_LR_MULT_FINAL})"
    )
    ...
    # If final already exists, don’t redo it

...
from datasetModel import Temporal1AlignedDataset, RISK_COLS
from modelArchitecture import RiskModel_alignedprior


EXP_ROOT = Path("/local/scratch/tpiltne/models/temporalModelAligned")
...
EARLY_STOP_PATIENCE = 3  # epochs without improvement on sel metric
...
def compute_pos_weight_from_train(
    train_ds: Temporal1AlignedDataset,
    ...
    if (
        hasattr(train_ds, "exam_groups")
        and isinstance(train_ds.exam_groups, list)
        and len(train_ds.exam_groups) > 0
    ):
        ...
        tmp_loader = DataLoader(train_ds, batch_size=32, shuffle=False, num_workers=0)
...
def enforce_encoder_bn_eval(model: nn.Module):
    try:
        enc = model.risk_prediction_model.encoder
    except Exception:
        enc = None
...
def evaluate(
    ...
    for imgs, delta_feat, has_prior_views, y, m in loader:
        ...
        out = model(imgs, delta_feat, has_prior_views)["risk_prediction"]
        logits_fused = out["pred_fused"]

        loss = masked_bce_with_logits(logits_fused, y, m, pos_weight=pos_weight)
        if USE_AUX_LOSSES:
            if "pred_cur" in out:
                loss = loss + float(AUX_W_CUR) * masked_bce_with_logits(out["pred_cur"], y, m, pos_weight=pos_weight)
            if "pred_pri" in out:
                loss = loss + float(AUX_W_PRI) * masked_bce_with_logits(out["pred_pri"], y, m, pos_weight=pos_weight)

        probs = torch.sigmoid(logits_fused).detach().cpu().numpy()
...
def train_one_config_grid(cfg: Dict[str, Any], run_dir: Path, device: torch.device) -> Dict[str, Any]:
    ...
    run_cfg.update(
        {
            ...
            "aux_losses": {"enabled": USE_AUX_LOSSES, "w_cur": AUX_W_CUR, "w_pri": AUX_W_PRI},
            "freeze_encoder_grid": bool(FREEZE_ENCODER_GRID),
            "dataset": "Temporal1AlignedDataset(split=train/val, strict_aligned=True)",
        }
    )
    (run_dir / "run_config.json").write_text(json.dumps(run_cfg, indent=2))

    train_ds = Temporal1AlignedDataset(split="train", strict_aligned=True)
    val_ds = Temporal1AlignedDataset(split="val", strict_aligned=True)
    ...
    for epoch in range(GRID_EPOCHS):
        ...
        for step, (imgs, delta_feat, has_prior_views, y, m) in enumerate(train_loader):
            imgs = imgs.to(device, dtype=torch.float32)
            delta_feat = delta_feat.to(device, dtype=torch.float32)
            has_prior_views = has_prior_views.to(device, dtype=torch.float32)
            y = y.to(device, dtype=torch.float32)
...
def main():
    ...
    if args.exp_dir is not None:
        ...
        stamp = (
            exp_dir.name.split("temporal_grid_then_final_")[-1]
            if "temporal_grid_then_final_" in exp_dir.name
            else datetime.now().strftime("%Y%m%d_%H%M%S")
        )
    else:
        stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        exp_dir = EXP_ROOT / f"temporal_grid_then_final_{stamp}"
        exp_dir.mkdir(parents=True, exist_ok=True)
    ...
    if not exp_cfg_path.exists():
        exp_cfg = {
            ...
            "dataset": "Temporal1AlignedDataset(strict_aligned=True)",
            "model": "RiskModel_alignedprior",
        }
    ...
    # keep summary copies at experiment root
    write_table_csv(table_rows, exp_dir / "grid_table.csv")
    (exp_dir / "best_config.json").write_text(json.dumps(best_overall, indent=2))
    (exp_dir / "final_best_summary.json").write_text(json.dumps(final_summary, indent=2))

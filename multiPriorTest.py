...
DEFAULT_EXP_ROOT = Path("/local/scratch/tpiltne/models/multiPrior")
...
def collate_multi_prior(batch):
    """
    Handles examples shaped like:
      (cur, pri, times, y, m) [+ optional group_id]
    ...
    m:    [HORIZONS]
    """
    if len(batch) > 0 and isinstance(batch[0], (list, tuple)) and len(batch[0]) == 6:
        cur_list, pri_list, t_list, y_list, m_list, gid_list = zip(*batch)
        have_gid = True
    else:
    ...
    cur = torch.stack(cur_list, dim=0)
    ...
    if Kmax == 0:
        ...
        if have_gid:
            gid = torch.as_tensor(gid_list)
            return cur, pri, t, pad, y, m, gid
        return cur, pri, t, pad, y, m

    pri_pad, t_pad, pad_mask = [], [], []
    for i in range(B):
        pri_i = pri_list[i]
        t_i = t_list[i]
        ...
        if Ki < Kmax:
            ...
            pm = torch.cat(
                [torch.zeros((Ki,), dtype=torch.bool), torch.ones((pad_amt,), dtype=torch.bool)],
                dim=0,
            )
        else:
    ...
    pri = torch.stack(pri_pad, dim=0)
    t = torch.stack(t_pad, dim=0).to(torch.float32)
    pad = torch.stack(pad_mask, dim=0)
...
def build_model_from_cfg(cfg: Dict[str, Any]) -> torch.nn.Module:
    ...
    unfreeze_final = cfg.get("unfreeze_encoder_final", None)
    if unfreeze_final is None:
        freeze_encoder = bool(cfg.get("freeze_encoder", True))
    else:
        freeze_encoder = not bool(unfreeze_final)

    return MultiPriorRisk(
...
def collect_logits_labels_masks(
    ...
    for batch in loader:
        if not isinstance(batch, (list, tuple)) or len(batch) < 6:
            raise ValueError("Unexpected batch format from collate_multi_prior.")

        cur, pri, t, pad, y, m = batch[:6]
        group_ids = batch[6] if len(batch) >= 7 else None
        ...
        out = model(cur, pri, t, pri_pad_mask=pad)
        if isinstance(out, dict) and "risk_prediction" in out:
...
def resolve_exp_dir(exp_dir: Optional[Path]) -> Path:
    ...
    if exp_dir.is_dir():
        if (exp_dir / "final_best").exists() or (exp_dir / "run_config.json").exists():
            return exp_dir

    if not exp_dir.exists():
        raise FileNotFoundError(f"exp_dir does not exist: {exp_dir}")

    runs = sorted(exp_dir.glob("multiPrior_grid_then_final_*"), key=lambda p: p.name)
    if not runs:
        raise FileNotFoundError(f"No multiPrior_grid_then_final_* runs found under: {exp_dir}")
    return runs[-1]
...
def main():
    ...
    eval_dir = exp_dir / f"eval_multiprior{run_tag}"
    ...
    test_ds = TemporalMultiPriorDataset(split="test")
